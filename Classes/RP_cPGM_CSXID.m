classdef RP_cPGM_CSXID < photon_CSXID & pgm_CSXID & phsource_CSXID
    properties
        r1      % Distance from the source to the collimating mirror (m)
        r2    	% Distance from the collimating mirror to the exit slit (m)
        r3  	% Distance from the vertically focusing mirror (VFM) to the exit slit (m)
        r4  	% Distance from the horizontally focusing mirror (HFM) to the exit slit (m)
        CMa 	% Incidence angle for the Collimating Mirror (CM) relative to normal (deg)
        VFMa 	% Incidence angle for the VFM relative to normal (deg)
        HFMa 	% Incidence angle for the HFM relative to normal (deg)
        es    	% Exit slit vertical opening (um)
        CMs  	% Collimating Mirror slope error in Sagittal direction (urad)
        VFMs    % VFM slope error in Sagittal direction (urad)
        HFMs    % HFM slope error in Sagittal direction (urad)
    ...
    methods
        ...
        function value=get.Vfm(RP_cPGM)
            value = 2.*RP_cPGM.r3.*RP_cPGM.VFMs.*1e-6.*cosd(RP_cPGM.VFMa);
        end
        ...
        function obj = RP_cPGM_CSXID(val)
